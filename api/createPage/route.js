...
export async function PUT(request) {
    ...
    const { url, metaDescriptions, metaTitle, metaKeywords, sections, mainTitle, pageType, shortContent, pageId } = JSON.parse(data);
    const pagemodel = await pageModel();
    if (!pagemodel) {
        return NextResponse.json({ status: false, message: "database error occured!" });
    }
    ...
    try {
        ...
        await pagemodel.update({
            url, metaDescriptions, metaTitle, metaKeywords, mainTitle, pageType, shortContent, bannerImage: bannerimage && bannerimage, sections: sections
        }, { where: { pageId } })
