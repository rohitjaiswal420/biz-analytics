...
export async function POST(request) {
    ...
    const { categoryId, altTag,serial } = JSON.parse(input.get('data'));
    ...
    try {
        ...
        await imagemodel.create({
            ...
            categoryId, altTag, imageUrl: imageUrl && imageUrl,serial
        })
...
export async function PUT(request) {
    const input = await request.formData();
    const file = input.get('file');
    const { categoryId, altTag,serial,imageId } = JSON.parse(input.get('data'));
    const imagemodel = await imageModel();
    if (!imagemodel) {
        return NextResponse.json({ status: false, message: "some error occured!" });
    }
