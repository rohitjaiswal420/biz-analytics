...
export default function Page({ categoryList }) {
    ...
    const createGallery = async (e) => {
        ...
        if (flag) {
            ...
            formData.append('data', JSON.stringify({ altTag, categoryId: category, serial, clientName: subCategory ,categoryName}));
            formData.append('file', imageUrl);

            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/imageClient`, { method: 'POST', body: formData })

            const res = await response.json();
            if (res.status) {
                sessionStorage.setItem('successMsg', 'Client Created Successfully');
                                    ...
                                    <form onSubmit={createGallery}>
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <select name="category" className="form-control" style={{ border: formValidation.category === 0 && '1px solid red' }} >
                                                ...
                                                {
                                                    categoryList.length > 0 && categoryList.map((item) => <option value={JSON.stringify({ id: item.categoryId, name: item.categoryName })} key={item.categoryId}>{item.categoryName.toUpperCase()}</option>)
                                                }
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <input type="text" className="form-control" id="alt-subCategory" placeholder="Enter Client Name" name='subCategory' style={{ border: formValidation.subCategory === 0 && '1px solid red' }} />
                                        </div>

                                        <div className="mb-3">
                                            <label className="form-label" htmlFor="menu-serial-input">Serial No.</label>
                                            <input type="number" className="form-control" id="menu-serial-input" placeholder="Enter Serial Number" name='serial' style={{ border: formValidation.serial === 0 && '1px solid red' }} />
                                        </div>
