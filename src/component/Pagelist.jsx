...
export default function Pagelist({ pageList, totalItems }) {
    ...
    const deleteRecords = async (pageId) => {
        ...
        if (!res.status) {
            ...
            sessionStorage.setItem('successMsg', 'Page deleted Successfully');
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/createPage?page=${idx}&name=${name}`);
        setName(name);

        const res = await response.json();
        if (res.status) {
            setpageLists(res.pagelist);
    ...
    const pagination = async (idx) => {
        ...
        if (idx > 0 && idx <= button) {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/page/createPage?page=${idx}&name=${name}`);
            setIdx(idx);
            const res = await response.json();
            if (res.status) {
                setpageLists(res.pagelist);
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><form><div className="row g-4 mb-3">
                            ...
                            <div className="col-sm-auto">
                                <div>
                                    <Link href="/admin/page/create-normal" className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add Normal Page</Link>
                                </div>
                            ...
                            <div className="col-sm-auto">
                                 ...
                                 <div>
                                    <Link href="/admin/page/create-service" className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add Service Page</Link>
                                </div>
                            </div>
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">ID</th><th className="text-uppercase" data-sort="customer_name">Place</th><th className=" text-uppercase" data-sort="email">Url</th><th className=" text-uppercase" data-sort="country">Page Type</th><th className=" text-uppercase" data-sort="date">Date</th><th className=" text-uppercase" data-sort="status">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {pageLists.length > 0 && pageLists.map((item, i) => <tbody key={item.pageId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{i + 1} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.bannerImage} alt='not found' />{item.mainTitle} </div></td><td className="email">{item.url}</td><td className="country">{item.pageType}</td><td className="date">{item.publishedDate} </td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <Link class="btn btn-sm btn-success edit-item-btn" href={item.pageType==='Service Page'?`/admin/page/update-service-page/${item.pageId}`:`/admin/page/update/${item.pageId}`}>Edit</Link>
                                        </div>
                                        <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn" onClick={() => openPopup(item.pageId)}>Remove</button>
                                        </div>
                                    </div>
                                </td></tr>
                            ...
                            <div className="noresult" style={{ display: button > 0 && "none" }}><div className="text-center"><lord-icon src="https://cdn.lordicon.com/msoeawqm.json" trigger="loop" colors="primary:#121331,secondary:#08a88a" style={{ "width": "75px", "height": "75px" }} /><h5 className="mt-2">Sorry! No Result Found</h5></div></div>
                                ...
                                {button > 0 && <div className="pagination-wrap hstack gap-2" style={{ "display": "flex" }}><button className="page-item pagination-prev" onClick={() => pagination(idx - 1)}>Previous</button>
                                    ...
                                    <ul className="pagination listjs-pagination mb-0">
                                        ...
                                        {/* href="#" data-i={1} data-page={8} */}

                                    </ul><button className="page-item pagination-next" onClick={() => pagination(idx + 1)}>Next</button></div>}
