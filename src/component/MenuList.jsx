...
export default function MenuList({ menuList }) {
    ...
    const addMenu = (e) => {
        ...
        e.preventDefault();
        const menuName=e.target.menu.value.trim();
        let menuUrl;
        if(pathType!=='select' && pathType!=="")
        {
            menuUrl=e.target.url.value.trim();
        }
        
        
        const currentMenu={
         menuName,menuUrl:menuUrl?menuUrl:'click',menuType:pathType==='select' || pathType==="" ? "":pathType,
        }

        const menu=[...menuList,currentMenu];
        console.log(menu);
            ...
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                            ...
                            <div className="col-sm-auto">
                                <div>
                                    <button className="btn btn-success" data-bs-toggle="modal"
                                        data-bs-target="#deletetable" type='button'><i className="ri-add-line align-bottom me-1" /> Add New menu</button>
                                </div>
                            </div>
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">S.No</th><th className=" text-uppercase" data-sort="invoice_id">ID</th><th className="text-uppercase" data-sort="customer_name">menu Name</th><th className=" text-uppercase" data-sort="country">Serial</th><th className=" text-uppercase" data-sort="status">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {menuList.length > 0 && menuList.map((item, i) => <tbody key={item.menuId} className="list form-check-all"><tr><td className="customer_name"><div className="d-flex align-items-center">{i + 1} </div></td><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.menuId} </a></td><td className="customer_name"><div className="d-flex align-items-center">{item.menuName} </div></td><td className="country">{item.serial}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <a class="btn btn-sm btn-success edit-item-btn" href={`/admin/image/menu/update/${item.menuId}`}>Edit</a>
                                        </div>
                                        <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn" onClick={() => openPopup(item.menuId)}>Remove</button>
                                        </div>
                                    </div>
                                </td></tr>
                        ...
                        </table><div className="noresult" style={{ display: menuList.length > 0 && "none" }}><div className="text-center"><lord-icon src="https://cdn.lordicon.com/msoeawqm.json" trigger="loop" colors="primary:#121331,secondary:#08a88a" style={{ "width": "75px", "height": "75px" }} /><h5 className="mt-2">Sorry! No Result Found</h5></div></div></div>
                                ...
                                <div className="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
                                    ...
                                    <form onSubmit={addMenu}>
                                        <div className="mb-3">
                                            ...
                                            <select name="type" className="form-control" onChange={(e) => setPathtype(e.target.value)}>
                                            ...
                                            {
                                                pathType === 'click' &&   <> <label className="form-label">
                                                Url
                                            </label><input
                                                type="text"
                                                className="form-control"
