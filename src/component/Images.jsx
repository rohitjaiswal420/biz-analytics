...
export default function Gallery({ galleryList, totalItems,clientId,clientName}) {
    ...
    const deleteMultipleRecords = async () => {
        ...
        checkboxlist.map((item, i) => item.checked && deleteditem.push(galleryLists[i].id))
        ...
        if (!res.status) {
            ...
            sessionStorage.setItem('successMsg','Images deleted Successfully');
    ...
    const openPopup = () => {
        ...
        let deleteditem = [];
        const checkboxlist = Array.from(document.querySelectorAll('.deleteinput'));
        checkboxlist.map((item, i) => item.checked && deleteditem.push(galleryList[i].id))
        setDeleteditems(deleteditem);
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/uploadImage?page=${idx}&name=${name}`);
        setName(name);

        const res = await response.json();
        if (res.status) {
            setGalleryLists(res.gallerylist);
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">{clientName} Images List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><form><div className="row g-4 mb-3">
                            <div className="col-sm-auto">
                                ...
                                {/* <div>
                                    <Link href="/admin/gallery/create" className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add New</Link>
                                </div> */}
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr> <th scope="row">
                            ...
                            <th className=" text-uppercase" data-sort="invoice_id">ID</th><th className="text-uppercase" data-sort="customer_name">Image</th><th className=" text-uppercase" data-sort="alt">Image Url</th><th className=" text-uppercase" data-sort="alt">Client Name</th>

                            {/* <th className=" text-uppercase" data-sort="action">Action</th> */}

                        </tr></thead>

                            {galleryLists.length > 0 && galleryLists.map((item, i) => <tbody key={item.GalleryId} className="list form-check-all"><tr>
                                ...
                                <td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.id} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.imageUrl} alt='not found' /> </div></td>
                                <td className='alt'>{item.imageUrl}</td>
                                <td className='alt'>{clientName}</td>
                                <td>
                                    <div class="d-flex gap-2">
                                        {/* <div class="edit">
                                            <a class="btn btn-sm btn-success edit-item-btn" href="#">Edit</a>
                                        </div> */}
                                        {/* <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn" onClick={() => openPopup(item.id)}>Remove</button>
                                        </div> */}
                                    </div>
                                </td>
                                </tr>
